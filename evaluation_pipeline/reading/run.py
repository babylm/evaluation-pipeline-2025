...
from evaluation_pipeline.reading.evaluation_functions import (
    get_p2,
    get_p2_enc_dec,
    get_p2_mlm,
    get_p2_mntp,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        type=pathlib.Path,
        help="Path to file containing the lambada dataset, we expect it to be in a JSONL format.",
    )
    parser.add_argument(
        "--model_path_or_name",
        required=True,
        type=str,
        help="The path/name to/of the huggingface folder/repository.",
    )
    parser.add_argument(
        "--backend",
        required=True,
        type=str,
        help="The evaluation backend strategy.",
        choices=["mlm", "mntp", "causal", "enc_dec"],
    )
    parser.add_argument(
        "--number_of_mask_tokens_to_append",
        default=3,
        type=int,
        help="When using either mlm or mntp, the number of mask tokens to append to approximate causal generation.",
    )
    parser.add_argument(
        "--revision_name",
        default=None,
        type=str,
        help="Name of the checkpoint/version of the model to test. (If None, the main will be used)",
    )
...
if __name__ == "__main__":
    ...
    if args.backend == "causal":
        ...
        p2_function = partial(
            get_p2_mlm, num_mask_tokens=args.number_of_mask_tokens_to_append
        )
    elif args.backend == "mntp":
        p2_function = partial(
            get_p2_mntp, num_mask_tokens=args.number_of_mask_tokens_to_append
        )
    elif args.backend == "enc_dec":
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if isinstance(row["prev_item"], str):
            try:
                prev_p, _ = p2_function(
                    row["prev_item"], row["prev_word"], model, tokenizer
                )
            except Exception:
                print(row)
                exit()
    ...
    with pred_file.open("w") as fj:
        ...
        for index, row in df.iterrows():
            preds_dict["reading"]["predictions"].append(
                {"id": index, "pred": row["pred"], "prev_pred": row["prev_pred"]}
            )
    ...
    with predictability_file.open("w") as fj:
        print(
            f"EYE TRACKING SCORE: {sum(report_values) / len(report_values):.2f}",
            file=fj,
        )
    ...
    for dv in variables:
        ...
        OLS_baseline = smf.ols(
            formula=dv
            + " ~ Subtlex_log10 + length + context_length + prev_length + prev_pred + Subtlex_log10:length + Subtlex_log10:context_length + Subtlex_log10:prev_length + Subtlex_log10:prev_pred + length:context_length + length:prev_length + length:prev_pred + context_length:prev_length + context_length:prev_pred + prev_length:prev_pred",
            data=temp,
        ).fit()
        R2_baseline = float(OLS_baseline.rsquared)
        aic_baseline = float(OLS_baseline.aic)
        ...
        OLS_model = smf.ols(
            formula=dv
            + " ~ Subtlex_log10 + length + context_length + prev_length + prev_pred + Subtlex_log10:length + Subtlex_log10:context_length + Subtlex_log10:prev_length + Subtlex_log10:prev_pred + length:context_length + length:prev_length + length:prev_pred + context_length:prev_length + context_length:prev_pred + prev_length:prev_pred + pred",
            data=temp,
        ).fit()
        is_sig = float(OLS_model.tvalues["pred"])
        the_p = float(OLS_model.pvalues["pred"])
